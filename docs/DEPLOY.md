...
Open Maestro **1.5.1** is a functional multi-agent orchestration layer with:
...
# Share dist/open_maestro-1.5.1-py3-none-any.whl
```

Each engineer runs the install script:

```bash
./install-ubuntu.sh /path/to/open_maestro-1.5.1-py3-none-any.whl
```
...
OPENAI=1 ./install-ubuntu.sh /path/to/open_maestro-1.5.1-py3-none-any.whl

# All SDK runtimes
OPENAI=1 CLAUDE_SDK=1 KIMI_ACP=1 ./install-ubuntu.sh /path/to/open_maestro-1.5.1-py3-none-any.whl
```
...
>
> ```bash
> CLAUDE_SDK=1 ./install-ubuntu.sh /path/to/open_maestro-1.5.1-py3-none-any.whl
...
v1.5.1 is backward-compatible with v1.3.0/v1.4.0/v1.4.1 milestone files. The
...
(v1.4.1), and chain mode defaulting to on in interactive mode (v1.5.1). After
upgrading the wheel, run `maestro --version` to confirm `1.5.1`.
